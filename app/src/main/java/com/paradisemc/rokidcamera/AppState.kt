...
data class CameraStatus(
    var mode: CameraMode = CameraMode.PHOTO,
    var recording: Boolean = false,
    var cameraOpen: Boolean = false
)
...
object AppState {
    ...
    fun getViewfinderMode(context: Context): ViewfinderMode {
        val value = context.getSharedPreferences("settings", Context.MODE_PRIVATE)
            .getString("viewfinderMode", ViewfinderMode.OFF.name) ?: ViewfinderMode.OFF.name
    ...
    fun setViewfinderMode(context: Context, mode: ViewfinderMode) {
        context.getSharedPreferences("settings", Context.MODE_PRIVATE)
            .edit().putString("viewfinderMode", mode.name).apply()
    }
}
